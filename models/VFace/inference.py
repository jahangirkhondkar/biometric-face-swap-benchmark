...
def main():
    ...
    parser.add_argument(
        "--reference_path",
        type=str,
        help="reference/source image path",
        default=""
    )
                ...
                with model.ema_scope():
                    ...
                    if not opt.skip_save:
                        for i, x_sample in enumerate(x_checked_image_torch):
                            ...
                            mask_save = 255.0 * rearrange(un_norm(inpaint_mask[i]).cpu(), "c h w -> h w c").numpy()
                            ...
                            ref_img_save = 255.0 * rearrange(all_img[2], "c h w -> h w c").numpy()
                            ref_img_save = Image.fromarray(ref_img_save.astype(np.uint8))
                            ref_img_save.save(os.path.join(sample_path, filename[:-4] + "_" + str(opt.seed) + "_ref.png"))
