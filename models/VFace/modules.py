...
LOCAL_CLIP_PATH = os.path.expanduser(
    "~/face_swap/VFace/REFace/pretrained/clip-vit-large-patch14"
)
...
class BERTEmbedder(AbstractEncoder):
    """Uses the BERT tokenizer model and adds transformer encoder layers"""
...
class FrozenCLIPImageEmbedder(AbstractEncoder):
    """Uses the CLIP vision transformer encoder from local pretrained files"""

    def __init__(self, version="openai/clip-vit-large-patch14"):
        super().__init__()
        self.transformer = CLIPVisionModel.from_pretrained(LOCAL_CLIP_PATH)
...
class FrozenCLIPTextEmbedder(AbstractEncoder):
    """Uses the CLIP text transformer from local pretrained files"""

    def __init__(self, version="openai/clip-vit-large-patch14"):
        super().__init__()
        self.transformer = CLIPTextModel.from_pretrained(LOCAL_CLIP_PATH)
        self.tokenizer = CLIPTokenizer.from_pretrained(LOCAL_CLIP_PATH)
        self.freeze()
...
class FrozenCLIPEmbedder(nn.Module):
    def __init__(self, version="openai/clip-vit-large-patch14"):
        ...
        self.model = CLIPModel.from_pretrained(LOCAL_CLIP_PATH)
        self.tokenizer = CLIPTokenizer.from_pretrained(LOCAL_CLIP_PATH)
